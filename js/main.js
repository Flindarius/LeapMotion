...
var sound = new Pizzicato.Sound({ 
    source: 'file',
    options: { path: '../assets/sound/test.wav' }
}, function() {
    console.log(sound);
    sound.loop = true
    sound.playing = true
...
controller.on('frame', (frame) => {
    ...
    frame.hands.forEach((hand) => {
        ...
        if (hand.pinchStrength >= 0.95){
            if (indexMeshTip.position.z <= 70 && indexMeshTip.position.z >= -60) {
    ...
    volumeR = Math.max((125 - potarDroite.position.z - 60) / 130 * 100,0)
    volumeL = Math.max((125 - potarGauche.position.z - 60) / 130 * 100,0)
    if (volumeR == 0){
